...
export default function RegisterVehicleScreen() {
  ...
  const [formData, setFormData] = useState<CarCreateDTO>({
    licensePlate: '',
    brand: '',
    model: '',
    color: '',
  ...
  const handleInputChange = (field: keyof CarCreateDTO, value: string) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleRegisterVehicle = async () => {
    ...
    try {
      ...
      // const response = await fetch('http://192.168.252.60:8080/api/v1/cars', {
      //   method: 'POST',
      //   headers: {
